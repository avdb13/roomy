...
export const registry: {
  ...
  bridge: new Command("bridge", async (
    [matrixSpaceAliasOrId, roomySpaceId], { log, intent, store, adminRoomId }
  ) => {
    ...
    if (matrixSpaceAliasOrId.startsWith('#')) {
      try {
        ...
        await intent.sendText(adminRoomId, `Failed to resolve room: ${err}`)
          .catch(() => { });
    ...
    if (!roomySpace) {
      await intent.sendText(adminRoomId, '🛑 Could not find a space with that ID. 😕')
        .catch(() => { });
    ...
    const entries =
      await store.getRoomyMappings(matrixSpaceId);
    ...
    try {
      ...
      await intent.sendText(adminRoomId, `Failed to get room state: ${err}`)
        .catch(() => { });

      return;
    }

    const matrixSpaceChildren = roomState.filter(
    ...
    if (matrixSpaceChildren.length === 0) {
      ...
      await intent.sendText(adminRoomId, `Matrix space ${matrixSpaceAliasOrId} has no children`)
        .catch(() => { });
    ...
    log.info(
      ...
      JSON.stringify(matrixSpaceChildren.map(({ roomId }) => ({ roomId })), null, 2)
    );

    let roomySpaceThreads: Awaited<
      ReturnType<typeof JazzWorker.getSpaceThreads>
    > | null = null;
    ...
    try {
      ...
      await intent.sendText(adminRoomId, `Failed to get Roomy space threads: ${err}`)
        .catch(() => { });
    ...
    log.info(
      ...
      JSON.stringify(roomySpaceThreads.map(({ thread: { id, name } }) => ({ id, name })), null, 2)
    );

    try {
      await store.storeRoom(
        new RoomyRoom(roomySpace), new MatrixRoom(matrixSpaceId)
      );
    } catch (err) {
      log.error('Failed to store space: %s', err);

      await intent.sendText(adminRoomId, `Failed to store space: ${err}`)
        .catch(() => { });
    ...
    for (const { thread } of roomySpaceThreads) {
      try {
        const matrixRoomId =
          await intent.resolveRoom(`#${thread.name}:kurosaki.cx`);

        if (
          !matrixSpaceChildren.map(({ roomId }) => roomId).includes(matrixRoomId)
        )
          throw new Error(
            `Room ${matrixRoomId} (${`#${thread.name}:kurosaki.cx`}) is not a space child of ${matrixSpaceId}`
          );

        const { membership }: { membership: string } =
          await intent.getStateEvent(matrixRoomId, 'm.room.member', intent.userId);
        ...
        await store.storeRoom(
          new RoomyRoom(thread, roomySpace.id), new MatrixRoom(matrixRoomId)
        );
      } catch (err) {
        log.error('Failed to resolve or store room: %s', err);

        await intent.sendText(adminRoomId, `Failed to resolve or store room: ${err}`)
          .catch(() => { });
    ...
    await intent.sendText(
      adminRoomId,
      "Roomy channel has been connected! 🥳"
    )
      .catch(() => { });
  })
};
...
export const parse = (message: string, context: Context) => {
  ...
  const [name, ...args] =
    message.slice(1).trim().split(/\s+/);

  const isCmd = (cmd: string): cmd is Name =>
    cmd in registry;

  const dispatch = <N extends Name>(
    name: N, args: string[], f: (args: Args[N]) => void,
  ) => {
    if (args.length <
      (registry[name].execute).length) return;
  ...
  if (isCmd(name))
    dispatch(
      name, args, (args) => registry[name].execute(args, context)
    );
}