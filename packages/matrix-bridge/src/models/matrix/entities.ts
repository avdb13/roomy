...
type ErrorPayload<T extends ErrorType> = T extends ErrorType.InvalidMatrixId
  ? {}
...
export class MatrixEntityError<T extends ErrorType = ErrorType> extends Error {
...
const validateDomain = (domain: string) => {
  if (!domain || domain.trim().length === 0) throw new MatrixEntityError(ErrorType.Empty);

  let endOfHost = domain.indexOf(':');
  if (endOfHost === -1) endOfHost = domain.length;

  if (domain.startsWith('[')) {
    const endOfIpv6 = domain.indexOf(']');

    if (endOfIpv6 === -1) throw new MatrixEntityError(ErrorType.InvalidDomain);
    ...
    const validPort =
      hasPort && domain[endOfIpv6 + 1] === ':' && isValidPort(domain.slice(endOfIpv6 + 2));

    if (hasPort && !validPort) throw new MatrixEntityError(ErrorType.InvalidDomain);
  }
  ...
  const validPort =
    hasPort && domain[endOfHost] === ':' && isValidPort(domain.slice(endOfHost + 1));
...
export class MatrixEntity {
  ...
  constructor(
    ...
    if (!fullId || fullId.trim().length === 0) throw new MatrixEntityError(ErrorType.Empty);

    if (fullId.length > ID_MAX_BYTES) throw new MatrixEntityError(ErrorType.MaximumLengthExceeded);

    if (!fullId.startsWith(sigil)) throw new MatrixEntityError(ErrorType.MissingLeadingSigil);

    const colon = fullId.indexOf(':');
    if (colon === -1) throw new MatrixEntityError(ErrorType.MissingColon);

    const localpart = fullId.slice(1, colon);
    const domain = fullId.slice(colon + 1);

    if (!domain || domain.trim().length === 0) throw new MatrixEntityError(ErrorType.InvalidDomain);
...
export class RoomId extends MatrixEntity {
  constructor(id: string) {
    ...
    if (id.includes('\0')) throw new MatrixEntityError(ErrorType.InvalidCharacters);
  }
}
