...
export class JazzWorker {
  ...
  public static async initialise(dataStore: DataStore) {
    ...
    if (this.account && this.profile) return;
  ...
  public static async getSpaceThreads(spaceId: string) {
    ...
    if (!allThreadsId) throw new Error('Space has no threads component');

    const allThreads = await sdk.AllThreadsComponent.load(allThreadsId, {
      resolve: {
        $each: {
          components: true,
        },
    ...
    const perAccountThreads = Object.values(allThreads.perAccount)
      .map(({ all }) => new Array(...all))
      .flat()
      .map(({ value }) => value);
    ...
    log.info(
      'Space threads: %s',
      JSON.stringify(
        perAccountThreads.map(({ id, name }) => ({ id, name })),
        null,
        2
      )
    );

    let loaded: Array<{ thread: RoomyEntity; component: ThreadComponent }> = new Array();

    for (const thread of Object.values(perAccountThreads)) {
  ...
  public static async getSpacePermissions(spaceId: string) {
    ...
    const spacePermissionsComponentId = space.components[sdk.SpacePermissionsComponent.id];

    if (!spacePermissionsComponentId) {
      log.error('No permissions component ID for Roomy space found: %s', space.id);

      return;
    }

    const permissions: SpacePermissionsComponent = await sdk.SpacePermissionsComponent.load(
      spacePermissionsComponentId
    );

    if (!permissions) {
      log.error('No permissions component for Roomy space found: %s', spacePermissionsComponentId);
  ...
  public static async getThreadComponent(thread: RoomyEntity) {
    ...
    const threadComponentId = thread.components[sdk.ThreadComponent.id];

    if (!threadComponentId) throw new Error('Thread has no component ID');

    const component = await sdk.ThreadComponent.load(threadComponentId, {
      resolve: {
        timeline: true,
      },
    });
  ...
  public static async listen(
    spaceId: string,
    onRoomyMessage: (message: RoomyMessage) => Promise<void>
  ) {
    ...
    for (const { component, thread } of threads) {
      ...
      component.timeline.subscribe(async (timeline: Timeline) => {
        ...
        for (const id of perAccount.sort((a, b) => b.madeAt.getTime() - a.madeAt.getTime())) {
          const message = await sdk.Message.load(id.value);

          if (!message) continue;

          {
            const lastSeenTimestamp = await this.dataStore.getLastSeenTimestamp(thread.id);

            if (message.createdAt.getTime() <= lastSeenTimestamp) continue;

            this.dataStore.saveLastSeenTimestamp(thread.id, message.createdAt.getTime());
          }
          ...
          if (!authorId) {
            log.warn('No author ID found for message: %s', JSON.stringify(message, null, 2));
          ...
          if (message.content) log.debug('Message body: %s', message.content);
